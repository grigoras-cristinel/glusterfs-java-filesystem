...
public class GlusterFileSystemProviderTest extends TestCase {
	...
	public void testNewFileChannel() throws Exception {
		...
		Set<? extends OpenOption> opts = new HashSet<>();
	...
	public void testNewFileChannelHelper() throws Exception {
		Set<? extends OpenOption> options = new HashSet<>();
		FileAttribute<?> attrs[] = new FileAttribute[0];
	...
	private void testReadAttributes_followLinks_helper(boolean success) throws Exception {
		...
		GlusterFileAttributes fakeAttributes = new GlusterFileAttributes(123, 234, 345, 12345l, 222111l, 121212l,
				212121, 2234231l);
	...
	private void copyFileAttributesHelper(boolean defaultPermissions) throws IOException {
		...
		blankStat.st_mode = 0644;
		stat eStat = new stat();
		long volptr = 1234L;
		mockStatic(GLFS.class);

		// when(GLFS.glfs_stat(volptr, mockPath.toString(),
		// blankStat)).thenReturn(0);
		doReturn(mockFileSystem).when(mockPath).getFileSystem();
		doReturn(mockFileSystem).when(targetPath).getFileSystem();
		doReturn("targetPath").when(targetPath).toString();
		doReturn(volptr).when(mockFileSystem).getVolptr();
		when(GLFS.glfs_stat(volptr, mockPath.toString(), eStat)).thenAnswer(new Answer<Integer>() {

			@Override
			public Integer answer(InvocationOnMock invocation) throws Throwable {
				stat tv = (stat) invocation.getArguments()[2];
				tv.st_mode = 0644;
				return 0;
			}
		});
		System.out.println("Acum dupa when contine e stat: " + eStat.toString());
		when(GLFS.glfs_stat(volptr, targetPath.toString(), eStat)).thenReturn(0);
		if (!defaultPermissions) {
			when(GLFS.glfs_chmod(volptr, targetPath.toString(), blankStat.st_mode)).thenReturn(0);
		}
		...
		if (!defaultPermissions) {
			verifyStatic();
			GLFS.glfs_chmod(volptr, targetPath.toString(), blankStat.st_mode);
		} else {
	...
	public void testReadSymbolicLink() throws Exception {
		...
		mockStatic(GLFS.class);
		// when(GLFS.glfs_readlink(volptr, pathString, content, (long)
		// length)).thenReturn(target.length());
		when(GLFS.glfs_readlink(isA(Long.class), isA(String.class), isA(byte[].class), isA(Long.class)))
				.thenReturn(target.length());
	 ...
	 *
	 * @param givenFileAttributes - used to test the case when the call to
	 * createDirectory includes a set of FileAttributes
	 *
	 * When errorHappens is true, the value of the second parameter doesn't
	 * matter.
	 */
	private void helperCreateDirectory(boolean errorHappens, boolean givenFileAttributes) throws IOException {
		...
		int mode = 0775; // using default file attribute to avoid testing
							// parseAttrs redundantly
		long volptr = 1234L;
