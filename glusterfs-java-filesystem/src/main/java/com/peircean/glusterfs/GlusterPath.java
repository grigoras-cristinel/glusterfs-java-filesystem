...
public class GlusterPath implements Path {
	...
	public Path subpath(int i, int i2) {
		if ((0 == i && parts.length <= 1 && parts[0].isEmpty()) || i < 0 || i2 < 0 || i >= parts.length
				|| i2 > parts.length || i > i2) {
	...
	public boolean startsWith(Path path) {
		...
		return ((absolute == otherPath.isAbsolute()) && (Arrays.equals(thisPrefix, otherPath.getParts())));
	}
	...
	public Path relativize(Path path) {
		...
		List<String> relativeParts = new LinkedList<>();
	...
	public File toFile() {
		File retval = null;
		if (absolute) {
			retval = new File(fileSystem.toString().concat(getString()));
		} else {
			retval = new File(fileSystem.toString().concat("/").concat(getString()));
		}
		return retval;
	}
	...
	void guardRegisterWatchDirectory() throws NotDirectoryException {
		if (!Files.isDirectory(this)) {
			throw new NotDirectoryException(
					"GlusterWatchService can only watch directories.  Not a directory: " + this);
		}
	}
	...
	void guardRegisterWatchService(WatchService watchService) {
		...
		if (!GlusterWatchService.class.equals(watchServiceClass)) {
			throw new UnsupportedOperationException(
					"GlusterPaths can only be watched by GlusterWatchServices. WatchService given: "
							+ watchServiceClass);
		}
	}
	...
	public boolean canRead() {
		try {
			getFileSystem().getProvider().checkAccess(this, AccessMode.READ);
			return true;
